...
/**
 * Owner-only "Move to team" on Project settings, and the modern Select.
 *
 * Signup needs an invite code, so this spec runs against seeded accounts:
 *   E2E_OWNER_EMAIL     the project's owner (projects.owner_id)
 *   E2E_NON_OWNER_EMAIL a project member who is not the owner (may hold the Owner role)
...
test.skip(!owner || !nonOwner || !password || !projectId, 'set E2E_OWNER_EMAIL, E2E_NON_OWNER_EMAIL, E2E_PASSWORD, E2E_PROJECT_ID');
...
test('the API refuses a move by a non-owner', async ({ page }) => {
  ...
  expect((await res.json()).error).toBe('only the project owner can move this project to another team');
});
