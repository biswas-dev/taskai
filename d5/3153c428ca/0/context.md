...
https://staging.taskai.cc/login did we destroy the db again? https://staging.taskai.cc/login

### Prompt 21

where is the wiki, it should a section within each project, e.g. https://staging.taskai.cc/app/projects/5/wiki

### Prompt 22

Build the wiki frontend UI
...
- This is a continuation session from previous wiki implementation work
- Previous session completed backend: Postgres, REST API, WebSocket, Yjs processor, search, MCP tools (Phases 0-6)
- All 7 commits were pushed to main, deployed to staging

**First ...

### Prompt 24

https://staging.taskai.cc/app/projects/5/wiki is redirecting to https://staging.taskai.cc/ please make sure these deployments are automated. We need to show version of DB, frontend and backend in the admin section and make sure when we deploy we see the status, build an API and MCP server for status and can check status after deploy
...
yes, fix the fix the staging deployment path issue so the version tracking deploys correctly

### Prompt 27

https://staging.taskai.cc/login anchoo2kewl@gmail.com comes back as invalid invalid email or password

what happened to data copy from prod data please ./scripts/server db-migrate  prod staging  --yes

### Prompt 28

i cannot invalid email or password

please check yourself anchoo2kewl@gmail.com and R5*D9LQuRyg0Q9
...
no make the script work

### Prompt 31

mok, looks much better but please fix the navbar, the spacing is all off, make it looking much more modern TaskAI
A dogfood project to self-improve!!
...
1. **User's Requests:**
   - Fix login issue on staging (database lost)
   - Build wiki frontend UI
   - Add version tracking system showing DB migration version
   - Fix staging deployment path issues
   - Fix database migration script
   - Modernize navbar spacing and layout
   - Deploy ch...
...
ok, it is working, let us promote to production, before that though the styles are good but they change position every time we click on Board, Wiki or Settings, should be at the same place if browser does not changhe

### Prompt 38

on production we get System Information
Refresh
Backend
Version
dev
...
production
Server Time
2/18/2026, 5:58:30 PM
 and on staging System Information
Refresh
Backend
Version
dev
...
please give me ./scripts/server promote to promote, never do it manually, this should trigger a github action, commit to memory, only way to update is to push, then github deployes to staging, then manually check then use ./scripts/server promote

### Prompt 41

staging shows as Environment
production

### Prompt 42

if I am on https://staging.taskai.cc/app/admin and system and do a page refresh, we go back to users. please add it in the url so we can stay on the page after refresh

### Prompt 43

please create a command for ./script/server watch [staging\prod] which will return the status and logs of the last gh build

### Prompt 44

X Run CI (22161934129) completed with 'failure'
...
1. **User's Initial Request**: The user reported that both production and staging show incorrect version information (version: "dev", git_commit: "unknown", build_time: "unknown", environment: "production" even on staging). They want proper semantic versioning (like 1.0.21) with auto-increme...
